...
import { ArrowLeftOutlined, ThunderboltOutlined, ApiOutlined, CheckSquareOutlined, SearchOutlined, UploadOutlined, DownloadOutlined, DeleteOutlined } from '@ant-design/icons';
...
const HotfixDetailsPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [details, setDetails] = useState<HotfixDetails | null>(null);
  ...
  const loadDetails = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await hotfixService.getDetails(id);
      setDetails(data);
    } catch (error) {
      message.error('Failed to load hotfix details');
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  const handleStatusChange = async (checkpoint: CheckpointAnalysis, newStatus: ChecklistStatus) => {
    if (!id) return;
    try {
      await hotfixService.updateChecklistStatus(id, checkpoint.id, { status: newStatus });
  ...
  const handleRemarkSave = async (checkpoint: CheckpointAnalysis, remark: string) => {
    if (!id) return;
    try {
      await hotfixService.updateChecklistStatus(id, checkpoint.id, { remark });
  ...
  const handleUploadAttachment = async (checkpoint: CheckpointAnalysis, file: File) => {
    if (!id) return;
    try {
      await hotfixService.uploadChecklistAttachment(id, checkpoint.id, file);
  ...
  const handleDownloadAttachment = async (checkpoint: CheckpointAnalysis) => {
    ...
    try {
      const blob = await hotfixService.downloadChecklistAttachment(id, checkpoint.id);
  ...
  const handleDeleteAttachment = async (checkpoint: CheckpointAnalysis) => {
    if (!id) return;
    try {
      await hotfixService.deleteChecklistAttachment(id, checkpoint.id);
  ...
  if (!details) {
    return <Empty description="Hotfix not found" />;
  }
  ...
  const filteredCheckpoints = details.checkpoints.filter(cp => {
    ...
    const matchesStatus = !checkpointStatusFilter || cp.hotfixStatus === checkpointStatusFilter;
    const matchesPending = !showOnlyPending || cp.hotfixStatus !== 'DONE';
  ...
  return (
    <div>
      <Button type="link" icon={<ArrowLeftOutlined />} onClick={() => navigate('/hotfixes')} style={{ marginBottom: 16, padding: 0 }}>
        Back to Hotfixes
      </Button>
      ...
      <Tabs activeKey={activeTab} onChange={setActiveTab}>
        <TabPane tab={<span><ThunderboltOutlined /> Overview</span>} key="overview">
          <Card>
            <Row gutter={[24, 16]}>
              <Col span={8}>
              ...
              <Col span={8}>
                ...
                <div>{details.deployedAt ? dayjs(details.deployedAt).format('YYYY-MM-DD HH:mm') : '-'}</div>
              </Col>
            </Row>
          </Card>
        </TabPane>
        ...
        <TabPane tab={<span><CheckSquareOutlined /> Checkpoint Analysis</span>} key="checkpoints">
          <Card>
            ...
            <Table
              ...
              size="small"
              rowClassName={(record) =>
                record.hotfixStatus === 'BLOCKED' ? 'checkpoint-blocked' :
                record.hotfixStatus === 'DONE' ? 'checkpoint-done' : ''
              }
            />
          </Card>
        </TabPane>
...
export default HotfixDetailsPage;
